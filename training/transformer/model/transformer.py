...
class Transformer(object):
  ...
  def __call__(self, inputs, targets, train):
    ...
    with tf.variable_scope("Transformer", initializer=initializer):
      ...
      encoder_outputs = self.encode(inputs, train, attention_bias=None)

      # Generate output sequence if targets is None, or return logits if target
      # sequence is known.
      decoder_outputs = self.decode(targets, encoder_outputs, train, attention_bias=None)
...
class PrePostProcessingWrapper(object):
  ...
  def __call__(self, train_phase, x, *args, **kwargs):
    ...
    if self.train:
      y = tf.nn.dropout(y, 1 - self.postprocess_dropout * train_phase)
    return x + y
...
class EncoderStack(tf.layers.Layer):
  ...
  def __init__(self, params, train):
    ...
    self.layers = []
    for _ in range(params["num_hidden_layers_enc"]):
  ...
  def call(self, encoder_inputs, train, attention_bias, inputs_padding):
    ...
    for n, layer in enumerate(self.layers):
      ...
      with tf.variable_scope("layer_%d" % n):
        with tf.variable_scope("self_attention"):
          encoder_inputs = self_attention_layer(train, encoder_inputs, train, attention_bias)
        with tf.variable_scope("ffn"):
          encoder_inputs = feed_forward_network(train, encoder_inputs, train, inputs_padding)
...
class DecoderStack(tf.layers.Layer):
  ...
  def call(self, decoder_inputs, encoder_outputs, train, decoder_self_attention_bias,
    ...
    for n, layer in enumerate(self.layers):
      ...
      with tf.variable_scope(layer_name):
        with tf.variable_scope("self_attention"):
          decoder_inputs = self_attention_layer(train, decoder_inputs, train, decoder_self_attention_bias, cache=layer_cache)
        with tf.variable_scope("encdec_attention"):
          decoder_inputs = enc_dec_attention_layer(train, decoder_inputs, encoder_outputs, train, attention_bias)
        with tf.variable_scope("ffn"):
          decoder_inputs = feed_forward_network(train, decoder_inputs, train)
